  ...
  async resetPassword(email: string, name: string, link: string) {
    try {
      await this.sendMail(
        { email, name },
        'Reset Password',
        `<h2>Вітаємо <b><i>${name}</i></b></h2> <p>Ми отримали запит на зміну пароля</p> <p>Якщо ви хочете змінити пароль,перейдіть за посиланням нижче:</p> <a href="${link}">-> ПОСИЛАННЯ <-</a> <br /> <br />
        <p>Краще не діліться посиланням з іншими, за для безпеки</p>
        <p> Якщо ви не зробили запит на зміну пароля, зв'яжіться з адміністрацією сайту</p>`,
      );
    } catch (error) {
